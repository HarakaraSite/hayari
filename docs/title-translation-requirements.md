...
各記事は Henji の同時実行枠を待つ前に、claim が自身のものかつ未読・非表示でないことを確認する。既読または非表示へ変わった記事は claim を外して `pending` へ戻し、Henji へ送らない。実行枠の待機中に状態が変わった場合は再確認せず、そのタイトルを送信してよい。削除済み記事には何も書き込まない。Henji 起動後に既読または非表示になった場合は、実行済みの結果を保存してよい。
